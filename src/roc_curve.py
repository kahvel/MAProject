...
def buildDataMatrix(data):
    ...
    for name in group_names:
        for avg in average:
            ...
            classification = classifiers.ThresholdRatioClassification(data, col_names[name], 0).classifyByAverage(avg)
            x.append(map(lambda x: x[2], classification))
            classification = classifiers.ThresholdDifferenceClassification(data, col_names[name], 0).classifyByAverage(avg)
            x.append(map(lambda x: x[2], classification))
...
if __name__ == '__main__':
    ...
    model = qda.QDA()
    # model = lda.LDA()
    train_labels = getTrueLabels(readData("..\\data\\test5_targets_2.csv"))
